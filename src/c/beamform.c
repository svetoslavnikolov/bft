        ...
        TSysParams* sys, double time,  double **rf_data, ui32 no_samples)
{
  ...
  if (atl->no_times==0){
     msgprint("\007 For the time being the dynamic focusing is ");
     msgprint("performed only on lines for which apodization is ");
     msgprint("specified.\n");
     return NULL;
  }
                      ...
                      double time,      ui32 no_samples)
{
   ...
   if (ftl->no_times == 0){
      eprintf("sum_lines_time:\n Error: "
              "There must be at least one focus time \n"
              "\tNext time call 'bft_focust_times' or 'bft_focus'\n");
      abort();
   }

   if (element1 > ftl->xdc->no_elements || element2 > ftl->xdc->no_elements){
      eprintf("sum_lines_time:\n"
              "Either the numer of element 1 or element 2 exceeds the number "
              "of elements of the associated transducer\n");
      abort();
   }
         ...
         ui32 element_no, TPoint3D *xmt)
{
   ...
   */
  if (flc->no_focus_time_lines != alc->no_apo_time_lines){
     eprintf("\007 beamform_image:\n");
     eprintf("Error : the number of apodization lines and the number of ");
     eprintf("focus lines must be the same \n");
     return NULL;
  }
  
  if (flc->no_focus_time_lines == 0){
     eprintf("\007 beamform_image:\n");
     eprintf("Error : the number of defined lines is 0\n");
     return NULL;
  }
  ...
  if (bf_lines == NULL){
     eprintf("\007 beamform_image:\n");
     eprintf("Errot : cannot allocate memory for the output image\n");
     return NULL;
  }
...

{
  ...
  for ( line_no = 0; line_no < flc->no_focus_time_lines; line_no ++ ){
     if (flc->ftl[line_no].dynamic == TRUE || flc->ftl[line_no].pixel == TRUE){
        eprintf("add_images:\n");
        eprintf("Error : unforunately, the addition is not supported \n");
        eprintf("        for dynamic or pixel based focusing .\n");
        abort();
     }
  }
